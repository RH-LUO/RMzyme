...
# Get unique cohorts
base_dir <- '/data/rluo4/RPMfunc/scMapping'
...
process_subdir <- function(cohort, subdir, allmeta, scRNA_annovar, use_split = TRUE) {
  ...
  samples <- intersect(samples, sc_Diseased_Samples$patient_id)
  ...
  select_cols <- c('ddt_id', 'patient_id', 'split_ids', 'tissue', 'disease',
                   ...
                   'Chromosome', 'Start_Position', 'End_Position', 'Reference_Allele', 'Tumor_Seq_Allele2', 'Hugo_Symbol'
, 'Variant_Classification', 'tx', 'exon', 'txChange', 'aaChange', 'Variant_Type', 'Func.refGene', 'Gene.refGene', 'VAF',
'CCF')
...
# # # test the function
# # cohort = 'GSE185381'
# # GSE185381_res <- process_cohort(cohort)
...
process_cohort <- function(cohort) {
  ...
  allmeta <- subset(sc_cell_summary, ddt_id == cohort) # sc_allmeta
  SNV_data <- file.path(SNV_dir, paste0(cohort, '_SCOMATIC_annovar.RData'))

  # Load reference data for column validation
  load(file.path(SNV_dir, 'GSE157376_SCOMATIC_annovar.RData'))
...
results <- lapply(combined_cohort, process_cohort)
#final_results <- do.call(rbind, results)
#saveRDS(final_results, file = file.path(indir, 'SComatic_maf_disco.rds'))
