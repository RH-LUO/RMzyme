...
print(paste0("current library: ", .libPaths()))
path <- c(.libPaths(), '/home/rluo4/R/x86_64-pc-linux-gnu-library/4.3', '/opt/R/4.3.1/lib64/R/library')
...
.libPaths(path)
print(paste0("changed library: ", .libPaths()))
library(ggplot2)
library(Seurat)
library(tidyverse)
library(NMF)
...
################### 需要修改绝对路径为当前工作路径
# 1. 读入当前所有数据集编号
loop_path <- "/data2/rluo4/EpiTrans/DataCollection/liver"#loop_path <- "/home/jjguo/xuhaixia/RPMfunc/disco/adjust_disco_split_tissue/skin"
...
for (j in 1:length(subfolder_names) ){#1:length(subfolder_names)) {
  ...
  for (i in 1:length(names(disease_list))) {
    ...
    RMP <- readxl::read_excel('/data2/rluo4/EpiTrans/DataCollection/RMP_update.xlsx')#read.csv("/home/jjguo/xuhaixia/RPMfunc/RMP_12types.csv",header = T)
    # add RMP+celltype
